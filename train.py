...
def main():
    ...
    training_data = ImageSequence(args.train_dataset, args.batch_size, args.length, captcha_symbols, args.width, args.height, args.is_audio)
    
    validation_data = ImageSequence(args.validate_dataset, args.batch_size, args.length, captcha_symbols, args.width, args.height, args.is_audio)
    ...
    try:
        #model.fit_generator(generator=training_data, validation_data=validation_data, epochs=args.epochs, callbacks=callbacks, use_multiprocessing=True)
        model.fit(training_data, epochs=args.epochs, callbacks=callbacks, verbose = 2, validation_data = validation_data, steps_per_epoch = None, use_multiprocessing = False)
